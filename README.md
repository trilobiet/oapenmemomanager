...
Output generated by the scripts is saved as files in directories, one for each client.


## Configuration

Configuration is set in a file named `application.properties`, in the same directory as the application executable jar. Use the template provided in `src/main/resources/application.properties.tpl`.

Among others these fields must be provided: 
...
* `path.customresources`   
   System path to resources for UI customization, e.g. `file:/home/oapen/oapenmemo/manager/customresources/` 
* `path.clientsurl`   
   (Public) URL where reports will be available, e.g. `https://memo.oapen.org/`. The manager does not depend in this URL, 
   it only serves to be able to navigate to the client directories from within the manager.
* `taskrunner.url`   
...
The admin user interface can be customized to display your logo and optionally adjust some of the colors to reflect your organization's identity.
...
- Copy `manager-x.y.z.jar` to the installation directory;
  ...
  (NGINX example below for 'memomanager.oapen.org', internal port 8084):
        ...
        server {
            ...
            location / {
                ...
                proxy_pass http://localhost:8084; # -> use the same port as in application.properties!
            }
        }
...
The database create script can be found [here](./dev/db/memo_create_MYSQL.sql).

**Before running the script replace all occurrences of `[name]_memo` with the actual intended database name!** E.g. `oapen_memo` or `doab_memo`.
